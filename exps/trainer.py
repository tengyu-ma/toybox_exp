...
class ToyboxTrainer:
    ...
    def get_learning_rate(self, epoch):
        for p in self.optimizer.param_groups:
            if self.net_name == 's2cnn':
                limits = [100, 200, 300]
                lr_decay = [1, 0.1, 0.01]
                for lim, decay in zip(limits, lr_decay):
                    if epoch < lim:
                        lr = decay * self.hyper_p.lr
                        break
                else:
                    lr = limits[-1] * self.hyper_p.lr

            elif self.net_name in ['svcnn', 'mvcnn', 'resnet18']:
                if epoch > 0 and (epoch + 1) % 10 == 0:
                    lr = p['lr'] * 0.5
                else:
                    lr = p['lr']
            else:
                raise ValueError(f'Unknown exp_name {self.net_name}')
            p['lr'] = lr
